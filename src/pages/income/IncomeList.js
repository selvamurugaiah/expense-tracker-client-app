    ...
    const fetchIncomeList = async () => {
      try {
        ...
        const response = await axios.get(
          `https://expense-tracker-app-9kz8.onrender.com/income/get-incomes?timePeriod=${timePeriod}&page=${currentPage}&userId=${userId}`,
          {
            headers: {
  ...
  const handleDeleteIncome = async (incomeId) => {
    try {
      const response = await axios.delete(
        `https://expense-tracker-app-9kz8.onrender.com/income/delete-income/${incomeId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
